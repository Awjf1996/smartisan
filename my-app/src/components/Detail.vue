            ...
            <div class="item-box">
                <div class="gallery-wrapper">
                    <div class="gallery">
                        <div class="thumbnail">
                            <ul>
                                <li v-for="(item,index) in detailData.stock_info[num].imgs"
                                :key="index"
                                :class="index==contNum?'on':''"
                                @click="changeContFn(index)" >
                                    <img :src="item">
                                </li>
                            </ul>
                        </div>
                        <div class="thumb">
                            <ul>
                                <li class="on"><img :src="detailData.stock_info[num].imgs[contNum]"></li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="banner">
                    <div class="sku-custom-title">
                        <div class="params-price">
                            <span><em>¥</em><i>{{detailData.stock_info[num].price}}</i></span>
                        </div>
                        <div class="params-info">
                            <h4>{{detailData.stock_info[num].title}}</h4>
                            <h6>{{detailData.stock_info[num].sub_title}}</h6>
                        </div>
                    </div>
                    <div class="sku-dynamic-params-panel">
                        <div class="sku-dynamic-params clear">
                            ...
                            <ul class="params-colors">
                                <li :class="index==num?'cur':''" v-for="(item,index) in detailData.stock_info" 
                                    :key="index"
                                    @click="changeFn(index)">
                                    <a :style="'background:'+item.detail.color">
                                        <i></i>
                                    </a>
                                </li>
                            </ul>
                        ...
                        <div class="sku-dynamic-params clear">
                            ...
                            <div class="params-detail clear">
                                <div class="item-num js-select-quantity">
                                    ...
                                    <span 
                                    :class="[detailData.stock_info[num].cartnum==1?'down-disabled':'','down']" 
                                    @click="cutFn"
                                    >-
                                    </span>
                                    <span class="num">{{detailData.stock_info[num].cartnum}}</span>
                                    <span 
                                    :class="[detailData.stock_info[num].cartnum==detailData.stock_info[num].limit_num?'up-disabled':'','up']" 
                                    @click="addFn"
                                    >+
                                    </span>
                    ...
                    <div class="sku-status">
                        <div class="cart-operation-wrapper clearfix">
                            <span class="blue-title-btn js-add-cart" @click="cartAdd"><a>加入购物车</a></span>
...
<script>
    ...
    export default {
        ...
        created () {
            ...
            var gData = JSON.parse(JSON.stringify(goodsData[idx]))
            gData.stock_info.forEach(function(obj){
                Vue.set(obj,'cartnum',1)
            })
            this.detailData = gData
        },
        methods: {
            changeContFn(i){
                this.contNum = i
            ...
            cutFn(){
                var n = this.detailData.stock_info[this.num].cartnum
                if(n>1){
                    this.detailData.stock_info[this.num].cartnum--
                }
            },
            addFn(){
                var n = this.detailData.stock_info[this.num].cartnum
                if(n<this.detailData.stock_info[this.num].limit_num){
                    this.detailData.stock_info[this.num].cartnum++
                }
            },
            cartAdd(){
                var obj = this.detailData.stock_info[this.num];
