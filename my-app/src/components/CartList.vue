                            ...
                            <div class="cart-group">
                                ...
                                <div class="cart-top-items" v-for="(item,index) in cartdata" :key="index">
                                    <div class="cart-items">
                                        ...
                                        <div class="operation">
                                            <a class="items-delete-btn" @click="delFn(index)"></a>
                                        </div>
                                        <div class="subtotal">¥ {{item.price*item.cartnum}}</div>
                                        <div class="item-cols-num">
                                            <div class="select js-select-quantity">
                                                <span 
                                                class="down" 
                                                :class="[item.cartnum>1?'':'down-disabled']" 
                                                @click="minuFn(index)">-</span>
                                                <span class="num">
                                                    <input type="text" style="display: inline-block;" v-model="item.cartnum">
                                                    <!-- <ul>
                                                        <li>{{item.cartnum}}</li>
                                                    </ul> -->
                                                </span>
                                                ...
                                                class="up" 
                                                @click="plusFn(index)" 
                                                :class="[item.cartnum<item.limit_num?'':'up-disabled']">+</span>
                    ...
                    <div class="cart-bar-operation">
                        <div>
                            <div class="choose-all js-choose-all">
                                <span 
                                class="blue-checkbox-new" 
                                :class="[allInfor.allCheck?'checkbox-on':'']" 
                                @click="allCheckFn(allInfor.allCheck)"><a></a></span>
                                全选
                            </div>
                    ...
                    <div class="shipping">
                        <div class="shipping-box">
                            <div class="shipping-total shipping-num">
                                <h4 class="">
                                    已选择 <i>{{allInfor.checkNum}}</i> 件商品
                                </h4>
                                <h5>
                                    共计 <i >{{allInfor.allNum}}</i> 件商品
                                </h5>
                            </div>
                            <div class="shipping-total shipping-price">
                                <h4 class="">
                                    应付总额：<span>￥</span><i >{{allInfor.checkPrice}}</i> 
                                </h4>
                        ...
                        <span class="jianguo-blue-main-btn big-main-btn js-checkout" :class="[allInfor.checkNum==0?'disabled-btn':'']">
                            <router-link :to="allInfor.checkNum==0?'':'/checkout'">现在结算</router-link>
                        </span>
                    </div>
                </div>
            </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            allCheckFn(bool){ //全选按钮点击
                // console.log(bool)
                this.$store.commit('allCheckMut',bool)
            },
            delFn(i){
                this.$store.commit('cartDelMut',i)
            },
            plusFn(i){
                this.$store.commit('plusMut',i)
            },
            minuFn(i){
                this.$store.commit('minuMut',i)
            },
        },
    }
</script>
