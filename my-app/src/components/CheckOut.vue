                    ...
                    <div class="box-inner js-checkout-address-panel ">
                        <div class="address-common-table js-multiple-address-panel">
                            <ul class="address-item-list clear js-address-item-list">
                                ...
                                v-for="(item,index) in addressInfor"
                                ...
                                :key="index">
                                    <div class="address-item">
                                        ...
                                        <div class="detail-section"> 
                                          {{item.location[0].area_name}} 
                                          {{item.location[1].area_name}} 
                                          {{item.location[2].area_name}} 
                                          <br> 
                                          {{item.address}} 
                                        </div>
                                    </div>
                                    <div class="operation-section">
                                        <span class="update-btn js-edit-address" @click="showPop(index)">修改</span>
                                        <span class="delete-btn js-delete-address" @click="addressDel(index)">删除</span>
                                    </div>
                                </li>
                                <li class="add-address-item js-add-address" @click="showPop(-1)">
                                    <p>使用新地址</p>
                                </li>
                            </ul>
                        </div>
                    ...
                    <div class="invoice-detail"> 发票抬头： 
                        <div class="radio-box"> 
                            <label> 
                                ...
                                <span class="blue-radio " :class="isBtnCheck?'blue-radio-on':''" @click="btn"><a></a></span>  个人
                            </label> 
                            <label> 
                                <input type="radio" class="hide"> 
                                <span class="blue-radio" :class="!isBtnCheck?'blue-radio-on':''"  @click="btn"><a></a></span>  单位
                            </label> 
                        </div> 
                        <div class="module-form-row form-item fn-hide js-invoice-title" v-if="!isBtnCheck"> 
            ...
            <div class="gray-box">
                ...
                <div class="box-inner ui-goods-cart">
                    ...
                    <div class="cart-table">
                        <div class="cart-group js-cart-group">
                            <div class="cart-items" v-for="(item,index) in checkCartData" :key="index">
                                ...
                                <div class="subtotal">
                                    <div class="subtotal-cell">  ¥ {{item.cartnum*item.price}}</div> 
                                </div>
                                <div class="goods-num">{{item.cartnum}}</div>
                ...
                <div class="box-inner"> 
                    <div class="order-discount-line"> 
                        <p> 商品总计： <span>¥ {{allInfor.checkPrice}}</span> </p> 
                ...
                <div class="box-inner"> 
                    <div class="last-payment clear"> 
                        <span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout">
                           <a>提交订单</a> </span> <span class="prices fn-right">应付金额： <em>¥ {{allInfor.checkPrice}}</em></span> 
                    </div> 
                </div>
            </div>
        </div>
        <address-pop v-if="isShow" @pop-click="realFidePop" :editnum="edit"></address-pop>
    </div>
...
<script>
    ...
    export default {
      ...
      computed:{
        checkCartData() {
          return this.$store.getters.checkCartData 
        },
        allInfor(){
          return this.$store.getters.allInfor
        },
        addressInfor(){
      ...
      methods: {
        btn() {
          this.isBtnCheck = !this.isBtnCheck
        },
        showPop(i){
          this.isShow = true
          this.edit = i
        ...
        addressDel(i){
          ...
          if(c){
            this.$store.commit('addressDelMut',i)
          }
        }
      },
    }
